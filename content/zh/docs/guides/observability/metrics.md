...
osm-edge 直接从 sidecar 代理（Pipy）中收集监控指标。通过这些指标，用户可以获取关于总体流量、流量中的错误、以及请求的响应耗时的信息。
...
- 修改 Prometheus 的 configmap，让它访问 pod/Pipy 的接口。osm-edge 自动给 pod 添加端口注解，负责将监听器配置推送给 pod，以供 Prometheus 来访问：
```yaml
...
为了实现 [SMI 指标规范][7]，osm-edge 中的 Pipy代理会生成如下 HTTP 流量的统计信息：
