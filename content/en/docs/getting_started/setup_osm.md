...
## Prerequisites
This demo of OSM {{< param osm_edge_version >}} requires:
...
Download the 64-bit GNU/Linux or macOS binary of OSM {{< param osm_edge_version >}}:
```bash
system=$(uname -s)
...
Download the 64-bit Windows OSM {{< param osm_edge_version >}} binary via Powershell:
```powershell
